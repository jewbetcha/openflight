...
def remove_selected_3v3(text: str) -> str:
    remove_segments = {
        ((30.52, 25.0), (31.0, 26.2), "F.Cu"),
        ((31.0, 26.2), (31.25, 28.3), "F.Cu"),
    }
    removed = 0
    for match in list(SEGMENT_RE.finditer(text)):
        ...
        if key in remove_segments or reverse_key in remove_segments:
            text = text.replace(match.group(0), "")
            removed += 1
    if removed != len(remove_segments):
        raise RuntimeError(f"expected to remove {len(remove_segments)} +3V3 segments, removed {removed}")
    return text
...
def build_routes() -> list[str]:
    ...
    routes.extend(
        [
            segment("+3V3", (30.52, 25.0), (31.4, 26.2)),
            segment("+3V3", (31.4, 26.2), (31.4, 27.8)),
            segment("+3V3", (31.4, 27.8), (31.25, 28.3)),
        ]
    )

    routes.extend(
        [
            segment("I2C_SCL", (31.7, 29.25), (32.4, 28.8)),
            via("I2C_SCL", (32.4, 28.8)),
            segment("I2C_SCL", (32.4, 28.8), (34.5, 29.99)),
            ...
            segment("I2C_SDA", (31.7, 28.75), (34.5, 27.99)),
            segment("I2C_SDA", (34.5, 27.99), (34.7, 27.4)),
            via("I2C_SDA", (34.7, 27.4)),
            segment("I2C_SDA", (34.7, 27.4), (28.0, 27.4), layer="In2.Cu"),
            segment("I2C_SDA", (28.0, 27.4), (28.0, 14.0), layer="In2.Cu"),
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: fix_i2c_pullups_and_3v3_clearance.py <board.kicad_pcb>", file=sys.stderr)
        return 2

    board = Path(sys.argv[1])
    text = board.read_text()
    text = remove_selected_3v3(text)
    text = remove_net(text, "I2C_SCL", expected_segments=6, expected_vias=2)
    text = remove_net(text, "I2C_SDA", expected_segments=6, expected_vias=2)
