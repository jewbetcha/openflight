...
def remove_old(text: str) -> str:
    ...
    for match in list(SEGMENT_RE.finditer(text)):
        if match.group(7) in NETS:
            ...
            removed_segments += 1
    if removed_segments != 19:
        raise RuntimeError(f"expected to remove 19 I2S segments, removed {removed_segments}")
    ...
    for match in list(VIA_RE.finditer(text)):
        if match.group(7) in NETS:
            ...
            removed_vias += 1
    if removed_vias != 6:
        raise RuntimeError(f"expected to remove 6 I2S vias, removed {removed_vias}")
    return text


def build_routes() -> list[str]:
    routes: list[str] = []
    # Fan out north from U1 to staggered vias outside the package body.
    routes.extend(
        [
            ...
            segment("I2S_DIN", (30.25, 28.3), (30.8, 27.4)),
            segment("I2S_DIN", (30.8, 27.4), (30.8, 26.2)),
            via("I2S_DIN", (30.8, 26.2)),
        ]
    )

    # Route on In2.Cu outside the Pi header pad field.
    routes.extend(
        [
            segment("I2S_LRCLK", (28.7, 26.2), (43.6, 26.2), layer="In2.Cu"),
            segment("I2S_LRCLK", (43.6, 26.2), (43.6, 50.18), layer="In2.Cu"),
            ...
            segment("I2S_DIN", (30.8, 26.2), (52.8, 26.2), layer="In2.Cu"),
            segment("I2S_DIN", (52.8, 26.2), (52.8, 52.72), layer="In2.Cu"),
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: reroute_i2s.py <board.kicad_pcb>", file=sys.stderr)
        return 2
