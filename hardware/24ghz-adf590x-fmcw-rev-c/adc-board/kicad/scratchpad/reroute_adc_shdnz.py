...
def remove_old(text: str) -> str:
    ...
    for match in list(SEGMENT_RE.finditer(text)):
        if match.group(7) == NET:
            ...
            removed_segments += 1
    if removed_segments != 6:
        raise RuntimeError(f"expected to remove 6 ADC_SHDNZ segments, removed {removed_segments}")
    ...
    for match in list(VIA_RE.finditer(text)):
        if match.group(7) == NET:
            ...
            removed_vias += 1
    if removed_vias != 2:
        raise RuntimeError(f"expected to remove 2 ADC_SHDNZ vias, removed {removed_vias}")
    return text


def build_routes() -> list[str]:
    return [
        segment(NET, (31.7, 30.75), (33.2, 30.75)),
        via(NET, (33.2, 30.75)),
        segment(NET, (33.2, 30.75), (51.0, 30.75), layer="In2.Cu"),
        segment(NET, (51.0, 30.75), (51.0, 24.78), layer="In2.Cu"),
        segment(NET, (51.0, 24.78), (48.26, 24.78), layer="In2.Cu"),
    ]
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: reroute_adc_shdnz.py <board.kicad_pcb>", file=sys.stderr)
        return 2
